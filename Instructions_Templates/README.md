...
3. Make sure that especially your larger folders are zip compressed.

4. Upload your data into the matching sub-collection that you created in A. e.g. your raw data can be deposited in `1_raw-data/`, your processed data in `2_processed-data/`, etc. 

Which data should be uploaded where? 

`1_raw-data/`: Contains the original, unmodified data as generated by experiments or external sources. Examples include sequencing outputs, field measurements, or external datasets.

Did you work with a consortium-wide datasets, publicly available data, or data that has already been published? 

Then you do not need to upload everything again! Storing data comes with a cost, so we try to avoid unnecessary duplication.

To make sure the user can find the raw data, you can use the [template_README_raw_data_access.md](https://github.com/melanorian/LK_data-work/blob/main/Instructions_Templates/template_README_raw_data_access.md). Here you can explain where raw data can be found and how to access them. 

2_processed-data/

Contains data that has been cleaned, filtered, transformed, or otherwise processed from the raw data. This is typically the input for downstream analyses. Think of e.g. an count matrix for RNAseq analysis. 
...
Contains documentation of how the data was generated and processed, including:

- laboratory protocols
- experimental workflows
- computational pipelines, scripts or references to code repositories
