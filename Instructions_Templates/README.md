...
You can now upload your files to YODA using your preferred method.

1. If needed, you can consult the full guide on working with YODA here or reach out for help:

[YODA guide](https://www.uu.nl/en/research/yoda/guide-to-yoda)

2. Whenever possible, follow these conventions:
...
- if possible, try to use one naming style consistently within a project  

Example:

`rna_seq_lettuce_low_nitrogen_t1_v1`

3. Make sure that larger folders are zip-compressed before uploading.

4. Upload your data into the appropriate sub-collection you created in section A. For example, raw data should be deposited in `1_raw-data/`, processed data in `2_processed-data/`, and so on.

## Which data should be uploaded where?

### `1_raw-data/`
Contains the original, unmodified data as generated by experiments or external sources. Examples include sequencing outputs, field measurements, or externally obtained datasets.

If you are working with consortium-wide datasets, publicly available data, or previously published data, you do not need to upload everything again. Storing data comes with a cost, so we aim to avoid unnecessary duplication.

Instead, ensure the data are clearly referenced. You can use the template [template_README_raw_data_access.md](https://github.com/melanorian/LK_data-work/blob/main/Instructions_Templates/template_README_raw_data_access.md) to document where and how the raw data can be accessed.

### `2_processed-data/`
Contains data that have been cleaned, filtered, transformed, or otherwise processed from the raw data. This is typically the input for downstream analyses, for example a count matrix for RNA-seq analysis.

### `3_results/`
Contains outputs generated from analyses, such as figures, tables, statistical results, models, and visualisations.

### `4_metadata-files/`
Contains all files needed to interpret the dataset, such as:
...
### `5_methods-and-protocols/`
Contains documentation of how the data were generated and processed, including:

- laboratory protocols  
- experimental workflows  
- computational pipelines  
- scripts or links to code repositories  
